...
Para avanzar con seguridad hacia asistencia conversacional y decision proactiva, las historias decisionales del DSS quedan cerradas con la siguiente evidencia minima:

| Prioridad | HU | Bloqueo | Gap exacto | Criterio de aceptacion literal | Evidencia minima |
| ---: | --- | --- | --- | --- | --- |
| 1 | HU21 | Cerrada | La recomendacion consolida accion, impacto y confianza por material/horizonte. | Para cada producto MVP, devuelve `comprar ahora`, `postergar` o `sin ventaja clara`, con horizonte, impacto ARS/%, MAPE y umbral aplicado. | Endpoint `/materiales/{material_id}/recomendacion-compra` y tests de recomendacion. |
| 2 | HU22 | Cerrada | La comparacion de estrategias queda como tabla accionable reutilizable. | Compara `100% ahora`, `100% futuro` y `50/50`, calcula costo total, diferencia ARS/%, estrategia ganadora y si supera umbral de decision. | Endpoint `/materiales/{material_id}/comparacion-estrategias-compra` y tests de estrategias. |
| 3 | HU28 | Cerrada | La decision presupuestaria integra criticidad + forecast + presupuesto como salida operativa. | Ingresa presupuesto, horizonte y materiales con cantidades; devuelve asignacion recomendada, presupuesto usado/restante, impacto esperado, criticidad y advertencias de confianza. | Endpoint `/compras/optimizar-presupuesto` y tests de optimizacion multi-material. |
| 4 | HU23 | Cerrada | La optimizacion presupuestaria queda validada con restriccion presupuestaria explicita. | La solucion respeta presupuesto, cantidades, no negatividad y explica que compra ahora/posterga por material. | Test reproducible donde el presupuesto limita la compra inmediata y el resultado no excede el presupuesto. |
| 5 | HU28b | Cerrada | Existe salida final que consolida recomendacion, explicacion, impacto economico, confianza y supuestos como cierre de DSS. | Para una obra o seleccion de materiales, devuelve recomendacion operativa trazable con accion, cantidades, impacto ARS/%, confianza, supuestos y advertencias. | Endpoint `/compras/recomendacion-operativa` y test de recomendacion trazable para los 3 productos MVP. |
| 6 | HU11 | Importante | La redaccion anterior asumia ingreso manual de precio actual, pero el flujo MVP toma precio base desde la serie del sistema y forecast con Prophet. | Permite seleccionar material y horizonte; usa el precio actual del sistema como base y devuelve precio futuro estimado con diferencia ARS/% y metrica de confianza. | Respuesta API o vista con `material`, `horizonte_meses`, `precio_actual_sistema`, `precio_proyectado` y variacion para los 3 productos MVP. |

Regla de cierre: una HU pasa a `Implementada` y `✅ Si` solo cuando funciona para los 3 productos MVP y entrega una salida accionable o verificable segun su criterio literal.
