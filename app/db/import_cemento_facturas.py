...
def import_facturas(db: Session) -> int:
    material = require_one(
        ...
        "No existe el material Cemento Portland. Ejecuta primero python -m app.db.seed.",
    )
    presentacion = require_one(
        ...
        "No existe la presentacion Bolsa 25 kg para Cemento Portland. Ejecuta primero python -m app.db.seed.",
    )
    fuente = require_one(
        db.scalar(select(Fuente).where(Fuente.nombre == "Factura compra")),
        "No existe la fuente Factura compra. Ejecuta primero python -m app.db.seed.",
    )
