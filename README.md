...
Repositorio base para cerrar la entrega con JHipster + tests + Docker/ELK + Ionic PWA + Jenkins.

## Contenido

- `jdl/e-commerce-monolith.jdl`: modelo importable con JHipster.
- `backend/`: pruebas unitarias y E2E de referencia.
- `docker/`: stack con PostgreSQL + ELK.
- `mobile/`: base Ionic/PWA que consume la API.
- `Jenkinsfile`: pipeline CI/CD para build, tests y push de imagen.
- `docs/IMPLEMENTACION_PASO_A_PASO.md`: guía de implementación.
- `docs/ESTADO_ENTREGA.md`: estado real y pendientes para cierre final.

## Flujo recomendado

1. Generar la app real con JHipster:
...
2. Ajustar tests a la app generada y ejecutar:
...
4. Configurar Jenkins con `DOCKERHUB_USER` y credencial `dockerhub-creds`.
<<<<<<< ours
<<<<<<< ours
<<<<<<< ours
>>>>>>> theirs
